...
- On that same mobile row, the date stays left-aligned and its badges (dues, event, note button) are pushed to the right edge of the column (`.sch-date-row` / `.sch-date-icons`), rather than the date and badges centering together as one block
