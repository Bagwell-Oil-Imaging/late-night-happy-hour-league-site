...
### Fixed
- `LeagueFormatInfo` - Correct Thursday night schedule text: practice now listed at 8:20 PM (was 7:50 PM), league play start at 8:30 PM (was 8:00 PM).
- `BylawsModal` - Bylaws lookup was hardcoded to season `'2025-2026'`, so the modal never followed the site's actual active season. Now resolves the season from `SeasonContext`: shows the current season's finalized bylaws while in-season, and the upcoming season's bylaws (whichever version is marked active for that season) while between seasons, falling back to the current season if no upcoming season is set.
- `SchedulePage` - Schedule table forced a horizontal scrollbar on mobile — `table-layout: auto` let the nowrap "View Matchups" button expand its column (and the whole table, past its `min-width: 640px`) well beyond the viewport, so users had to scroll right just to reach the button. Below 768px the table now switches to `table-layout: fixed` with `min-width: 0` and tighter, wrap-friendly column widths, so it fits the viewport with no horizontal scrollbar. Also adds a "Matchups" column header above the action buttons, previously blank.
- `ContactPage` - The "Express Interest" title read flush against the panel's left edge on mobile once the panel's horizontal padding was cut for form width — adds its own left padding back on the title only, so the iframe still gets the full width.
- `ContactPage` - The Express Interest Google Forms iframe was clipped on mobile — its fixed 1357px height was tuned for the desktop-width layout, but the embedded form reflows taller as its own width narrows (question text wraps to more lines), so the submit button was unreachable below ~860px. Also narrows the form panel's mobile padding twice over (down to `var(--space-1)` horizontally at ≤480px) so the form fills nearly all of the available width. Went through several rounds tuning the mobile iframe height after real-device feedback: a viewport-relative `78vh` cap traded the clip for a visible internal scrollbar (Google Forms doesn't support iframe-resizer/postMessage auto-sizing, so that can't be fixed on our end), and the first pass at fixed-height tiers still overshot and stranded Google's floating "?" help icon in a dead gap — Puppeteer's default desktop UA at a narrow viewport doesn't reflect what a real phone actually gets served. Landed on four fixed-height tiers (1440/1450/1510/1570px), re-measured with `page.emulate(KnownDevices['iPhone 12'])` (mobile UA + touch) at each breakpoint's actual effective iframe width — no scrollbar, no dead space.
...
### Added
- `LeagueMap` + `VenueSettings` - Off-season landing page now shows a Google Maps embed of the upcoming season's bowling center, positioned between the Week 1 countdown and the "Join the League!" button; hidden entirely when no address is set. Uses the key-less `google.com/maps?...&output=embed` URL, so no Maps API key is required. The address (plus bowling center name) is admin-editable per season via a new **Venue** card in Site Settings → Season Details, backed by a new `leagueConfig.bowlingCenterAddress` field and a `set-venue` local-admin-bypass operation.
- `SeasonPlaceholder` - New reusable component shown on `StandingsPage`, `MatchupsPage`, `SchedulePage`, `TeamsPage`, `BowlersPage`, and `LanesPage` when `seasonActive` is false, replacing each page's normal content so stale prior-season data can no longer be reached via the hamburger menu while the league is between seasons. States what the page will show once the season starts and links back home.
- `SchedulePage` - Skip weeks in the monthly mini-calendars now show a red X drawn over the date, with the skip reason in a tooltip on hover (desktop) or tap/keyboard focus (mobile) instead of relying solely on the native `title` attribute, which doesn't work on touch.
- `SchedulePage` - Play-week calendar cells show the league week number in brackets below the date (replacing the plain status dot), colored to match completed/upcoming status.
- `SchedulePage` - Skip weeks' schedule table row now repeats the skip reason beneath the date on mobile — the Notes column (where it normally lives) is hidden below 768px, so it was silently dropped from the "OFF" rows on phones.
- `SchedulePage` - Between seasons, preview the upcoming season's calendar and week table (`settings/global.upcomingSeasonYear`) instead of falling back to the generic placeholder, with a banner noting the season hasn't started; falls back to `SeasonPlaceholder` when no upcoming season year is set, or none has been staged yet. Calendar month range is now derived from the displayed season's own `scheduleWeeks` dates instead of a hardcoded Sept–May span, so this works for any season year. `WeekMatchupsModal` now takes a `seasonYear` prop instead of hardcoding `'2025-2026'`, so its matchup/team/config lookups stay correct for whichever season `SchedulePage` is displaying.
